<?php 
	...
	*/
		...
		for($i = 1; $i <= $iteraciones; $i++)
				{
					...
					$pdf->Cell(280,6,utf8_decode('DENOMINACIÓN DEL MATERIAL ELECTORAL'),1,1,'C');

					$pdf->SetFont('Arial','B',45);
	# MultiCell(float w, float h, string txt [, mixed border [, string align [, boolean fill]]])
					$pdf->MultiCell(280,30,$requestPdfReq['producto'],1,'C',0);

					#$pdf->Cell(280,60,$requestPdfReq['producto'],1,1,'J');
	...
	# Cell(float w [, float h [, string txt [, mixed border [, int ln [, string align [, boolean fill [, mixed link]]]]]]])
					...
					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(30,8,utf8_decode('UMA'),1,0,'C');
					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(45,8,$requestPdfReq['uma'],1,0,'C');
					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(30,8,utf8_decode('MEDIDA'),1,0,'C');
					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(95,8,utf8_decode('--'),1,1,'C');

					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(25,8,utf8_decode('FAMILIA'),1,0,'C');
					$pdf->SetFont('Arial','B',10);
					$pdf->Cell(130,8,$requestPdfReq['familia'],1,0,'C');
					$pdf->Cell(30,8,utf8_decode('CLASE'),1,0,'C');
			...
			public function getSelectProducto()
		{
			...
			$arrData = $this->model->getSelectProcucto();
			if(count($arrData) > 0){
				for ($i=0; $i < count($arrData) ; $i++) { 
