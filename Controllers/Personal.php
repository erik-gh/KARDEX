	...
	class Personal extends Controllers
	{
		...
		public function setPersonal()
		{
			...
			if($_POST){
				if(empty($_POST['dni']) || empty($_POST['apellido']) || empty($_POST['nombre']) || empty($_POST['cargo']) || empty($_POST['gerencia'])){
					...
					$intFechaIn     = strClean($_POST['FechaIngreso']);
					...
					if($intControl == 0){
						$requestPersonal	= $this->model->insertPersonal($strDNI, $strNombre, $intCargo, $intGerencia,$intTurno,$intFechaIn, $intUserSession, $strImagen);
					}else{
						if($intListaBlack == 3){
							$requestPersonal	= $this->model->updatePersonal($intIdPersonal, $strDNI, $strNombreC, $intCargo, $intGerencia , $intTurno,$intFechaIn,$intUserSession, $intListaBlack, $strImagen);
						}else{
							$requestPersonal	= $this->model->updatePersonal($intIdPersonal, $strDNI, $strNombreC, $intCargo, $intGerencia,$intTurno, $intFechaIn, $intUserSession, $intEstado, $strImagen);
						}
					}
		...
		public function plantillaExcel()
		{
			if (PHP_SAPI == 'cli')
				...
				$titulosColumnas = array(' D.N.I. ',' APELLIDOS Y NOMBRES ', ' CARGO ', ' GERENCIA ', ' ESTADO ', ' RESPONSABLE ', ' Nº CARGO ', ' CARGO' );
				...
				$objPHPExcel->setActiveSheetIndex(0)
                    		...
                    		->setCellValue('O2', $titulosColumnas[6])
                    		->setCellValue('P2', $titulosColumnas[7]);
                ...
                $arrDataResponsable = $this->model->selectcboResponsable();
		       	
		        $valoresResponsable	= '';
		        $responsables 		= '';
		        for ($i=0; $i < count($arrDataResponsable); $i++) { 
		        	# code...
		        	$valoresResponsable .= $arrDataResponsable[$i]['id_responsable'].',';
		        	$responsables .= $arrDataResponsable[$i]['id_responsable'].' - '.$arrDataResponsable[$i]['responsable'].',';
		        }

                $valoresResponsable	= trim($valoresResponsable,',');
                $responsables 		= trim($responsables,',');
                $resultResponsables	= str_replace(',', "\n", $responsables);
                for ($i=3; $i < 500; $i++) { 
		                ...
		                $objValidation->setError('El valor no está en la lista.');
		                $objValidation->setPromptTitle('Elegir de la lista');
		                $objValidation->setPrompt($resultResponsables);
		                $objValidation->setFormula1('"'.$valoresResponsable.'"');
                }
						...
						if (isset($_FILES['archivo'])) {
							...
							if(($sheet->getCell("Z200")->getValue()) == md5('Personal@GGE')) {
								...
								for ($row = 3; $row <= $highestRow; $row++){ 
									...
									$F3 = $sheet->getCell("F".$row)->getValue();
									...
									$intResponsable	= intval(strClean($F3));
									...
									if( $A3 != '' ){
										...
										if(strlen(strClean($A3)) == 8){
											if(!in_array($A3, $valoresDNI)){
												...
												$validos .= $A3.',';
												//insertar datos
												$requestImportar = $this->model->importarPersonal($strDNI, $strNombre, $intCargo, $intGerencia, $intUserSession, $intEstado, $intResponsable);
		...
		public function getSelectGerencia()
		{
			...
			public function getSelectturno()
		{
			$htmlOptions = '<option value="">[ Seleccione Gerencia ]</option>';
			$arrData = $this->model->selectcboturno();
			if(count($arrData) > 0){
				for ($i=0; $i < count($arrData) ; $i++) { 
					$htmlOptions .='<option value="'.$arrData[$i]['id_horario'].'"> '.$arrData[$i]['turno'].'</option>';
				}
			}
			echo $htmlOptions;
			die();
