...
function cboturno(){
	$.ajax({
	    type: "GET",
	    async : false,
	    url: base_url+'/Personal/getSelectturno', 
	    success: function(data, textStatus, jqXHR){
	    	if(jqXHR.status == 200){
	            //console.log(data);
	            $("#cboturno").selectpicker('destroy');
	            $("#cboturno").html(data).selectpicker('refresh');
	        }
	    }
	});
}
...
$("#form_registerPersonal").submit(function() {
	...
	requestPersonal["turno"]			= $("#cboturno").val();
	requestPersonal["FechaIngreso"]	    = $("#txtfechadeingreso").val();
      	...
      	success: function(data, textStatus, jqXHR){
	       	...
	       	if(jqXHR.status == 200){
		        ...
		        if(data.status){
			        ...
			        $('#cboturno').selectpicker('val',data.data.id_horario);
			        $('#txtfechadeingreso').val(data.data.FechaIngreso).addClass('vld');
