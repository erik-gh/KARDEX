...
function cboCentroConsumo(){
	$.ajax({
	    type: "GET",
	    async : false,
	    url: base_url+'/Usuario/getSelectCentroConsumo', 
	    success: function(data, textStatus, jqXHR){
	    	if(jqXHR.status == 200){
	            //console.log(data);
	            $("#cboconsumous").selectpicker('destroy');
	            $("#cboconsumous").html(data).selectpicker('refresh');
	        }
	    }
	});
}
...
$("#form_registerUsuario").submit(function() {
	...
	requestUsuario["cconsumo"]			= $("#cboconsumous").val();
...
function cancelUsuario(){
	...
	$('#cboconsumous').selectpicker('refresh');
	mostrarContrasena();
}
...
$("#form_updatePassword").submit(function(){
	...
	requestNewPassword["cconsumo"]			= $("#cboconsumous").val();
