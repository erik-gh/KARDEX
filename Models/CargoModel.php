...
class CargoModel extends Mysql
{
	...
	public function insertCargo(string $cargo, int $userSession)
	{
		...
		if(empty($requestCargo)){
			...
			$query = "INSERT INTO cargo(cargo, remuneracion, user_create, cantidad, estado) VALUES(?,1,?,0,1)";
			$arrData = array($this->strCargo, $this->intUserSession);
	...
	public function updateCargo(int $idCargo, string $cargo,  int $userSession, int $estado)
	{
		...
		if(empty($requestCargo)){
			...
			$query = "UPDATE cargo SET cargo = ?,  user_update = ?, estado = ? WHERE id_cargo = $this->intIdCargo";
			$arrData = array($this->strCargo,  $this->intUserSession, $this->intEstado);
